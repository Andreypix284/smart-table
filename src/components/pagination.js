...
export const initPagination = ({ pages, fromRow, toRow, totalRows }, createPage) => {
    ...
    // удаляем оригинальный элемент-шаблон
    return (data, state, action) => {
        // @todo: #2.1 — посчитать количество страниц, объявить переменные и константы
        const rowsPerPage = parseInt(state.rowsPerPage) || 10;                        // будем часто обращаться, чтобы короче записывать
        const pageCount = Math.ceil(data.length / rowsPerPage) || 1;        // число страниц округляем в большую сторону
        ...
        // Ограничиваем текущую страницу, если она выходит за пределы
       ...
       const startRow = (page - 1) * rowsPerPage + 1;
        ...
        fromRow.textContent = data.length > 0 ? startRow : 0;
        toRow.textContent = data.length > 0 ? endRow : 0;
        totalRows.textContent = data.length;
                                       
        // @todo: #2.2 — посчитать сколько строк нужно пропустить и получить срез данных
        const skip = (page - 1) * rowsPerPage;
        return data.slice(skip, skip + rowsPerPage); //0, 10 здесь в скобках должно быть
    }
}