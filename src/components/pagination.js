...
export const initPagination = ({ pages, fromRow, toRow, totalRows }, createPage) => {
    ...
    return (data, state, action) => {
        ...
        const rowsPerPage = state.rowsPerPage || 10;                        // будем часто обращаться, чтобы короче записывать
        const pageCount = Math.ceil(data.length / rowsPerPage) || 1;        // число страниц округляем в большую сторону
        let page = state.page || 1;                                        // страница переменной, потому что она может меняться при обработке действий позже
